...
def exit_cleanup(scan_file, scans=None):
    scan_file.close()
    if scans:
        for scan in scans:
            scan.close()
...
def run(fullmap):
    ...
    if proj == 'CREATE-IP':
        ...
        for var in variables:
            ...
            scan = scans[-1].name
            autstr = autoc_command + ' --out_pretty --out_json {} --files "{}/{}_*.nc"'
    ...
    if proj == 'CREATE-IP':
        for scan in scans:
            try:
                if third_arg_mkd:
                    out_json_data = mkd.run([map_json_data, scan.name, data_node, index_node, replica, data_roots, globus, dtn, silent, verbose, json_file])
                else:
                    out_json_data = mkd.run([map_json_data, scan.name, data_node, index_node, replica, data_roots, globus, dtn, silent, verbose])
                datasets.append(out_json_data)
            except Exception as ex:
    ...
    if proj == 'CREATE-IP':
        for data in datasets:
            try:
                up.run([data, index_node, cert, silent, verbose])
            except Exception as ex:    
    ...
    if proj == 'CREATE-IP':
        for data in datasets:
            try:
                ip.run([data, index_node, cert, silent, verbose])
            except Exception as ex:
